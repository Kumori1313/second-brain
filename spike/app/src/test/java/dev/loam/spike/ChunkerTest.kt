...
class ChunkerTest {
    ...
    fun oversizedBlockIsSplitBelowCeiling() {
        ...
        val chunks = Chunker.chunk(row.repeat(2000))
    ...
    fun ceilingHoldsWithNoWhitespaceToBreakOn() {
        ...
        val chunks = Chunker.chunk("x".repeat(20_000))
    ...
    fun realVaultProducesRetrievableChunkSizes() {
        ...
        vault.walkTopDown()
            ...
            .forEach { f ->
                files++
                sizes += Chunker.chunk(f.readText()).map { it.length }
            }
